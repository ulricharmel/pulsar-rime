...
is a Tigger model (-f switch must be specified to allow overwriting), or else a new filename is generated.""")
    ...
    group.add_option("--bprefix", type="string", default="beamgain")
    ...
    if pb:
        ...
        if not (options.app_to_int or options.int_to_app):
            ...
            beamgains = numpy.ones((len(sources), len(pa_range)))
                ...
                if r is not None:
                    if fitsBeam:
                        ...
                        # new-style averaging of Mueller matrix
                            ...
                            # hack to save the beamgains, no averaing just return the [0,0]
                            beamgains[s,:] = numpy.asarray(muellers)[:,0,0]

            numpy.save(options.bprefix+".npy", beamgains)
